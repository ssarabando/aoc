...
func TestPartOneWithSmallDataset(t *testing.T) {
	overlapping_pairs := PartOne("input_test.txt")
	if overlapping_pairs == 0 {
		t.Fatalf(`Number of test overlapping pairs should be 2. Was %d.`, overlapping_pairs)
	}
}

func TestPartOneWithRealDataset(t *testing.T) {
	overlapping_pairs := PartOne("input.txt")
	if overlapping_pairs == 0 {
		t.Fatalf(`Number of test overlapping pairs should be ?. Was %d.`, overlapping_pairs)
	}
}
